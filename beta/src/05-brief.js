...
// The brief's local mode (morning/evening) drives which day the weather chip
// shows: morning → today, evening → tomorrow ("Tmrw …"), matching the server's
// buildWeatherChip. Read it off the loaded brief, not the wall clock.
function briefLoadedMode() {
  return _briefState.brief?.structured?.mode || _briefState.brief?.mode || 'morning';
}

// Format the chip text from a beta-weather payload for the given mode — the
// single source for both the auto-refresh and the tap-to-expand chip update,
// so neither one reverts an evening "Tmrw" chip back to today.
function briefWeatherChipText(data, mode) {
  const c = data.current || {};
  const day = (mode === 'evening') ? (data.tomorrow || {}) : (data.today || {});
  if (day.high_f == null || day.low_f == null) return null;
  const emoji = day.emoji || (mode === 'evening' ? '' : c.emoji) || '';
  const temps = `${Math.round(day.high_f)}°/${Math.round(day.low_f)}°`;
  const block = mode === 'evening' ? `Tmrw ${temps}` : temps;
  return `${emoji ? emoji + ' ' : ''}${block}`;
}

// The chip text rendered above comes from the brief's cached snapshot, which can
// be hours old (and may have been generated by a now-replaced weather provider).
// On every brief render we do one light live read (same endpoint the tap-to-
// expand modal uses) and rewrite the chip in place, so it always reflects the
// current provider/conditions instead of reverting to the stale baked-in value.
async function briefRefreshWeatherChip() {
  if (!document.querySelector('.brief-weather-chip')) return;  // no chip on screen
  try {
    ...
    if (!res.ok || !data.ok || !data.has_location) return;
    const txt = briefWeatherChipText(data, briefLoadedMode());
    if (!txt) return;
    const el = document.querySelector('.brief-weather-chip');   // re-query post-await
    if (!el) return;
    el.textContent = txt;
...
function briefRecapHTML(recap, structured) {
  ...
  const buildRows = (col) => {
    ...
    if (col.events != null && col.events > 0 && (col.label === 'Today' || col.label === 'Tomorrow')) {
...
function briefRender() {
  ...
  briefStartRotation();
  briefRefreshWeatherChip();   // keep the chip live, not the cached snapshot
}
...
async function homeBriefLoad() {
  ...
  _briefInflight = (async () => {
    ...
    try {
      ...
      // no structured.recap. Likewise, a row written before the state-circle
      // redesign has no structured.rotation (and still carries the old
      // activity/readiness stat rows + a stale weather snapshot). On first
      // view we force a fresh generation so the user sees the new layout
      // immediately instead of waiting for the top-of-hour cron tick.
      // Idempotent — once the row has both recap and rotation, this branch
      // never fires again for that brief_date+mode.
      if (data && data.structured && (!data.structured.recap || !data.structured.rotation)) {
...
function briefRenderWeather(d) {
  ...
  // Mode-aware so an evening "Tmrw" chip isn't reverted to today on tap.
  try {
    const chipEl = document.querySelector('.brief-weather-chip');
    const txt = briefWeatherChipText(d, briefLoadedMode());
    if (chipEl && txt) chipEl.textContent = txt;
  } catch (e) { /* non-fatal */ }
