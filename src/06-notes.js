...
function renderNotesSidebar() {
  ...
  let html = `<div class="ns-section">
    ...
    <div class="ns-item${notesSidebarView==='starred'?' active':''}" data-notes-action="view" data-view="starred">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 17v5M9 10.76V6a2 2 0 1 1 4 0v4.76l3 1.54V15H6v-2.7l3-1.54z"/></svg>
  ...
  <div class="ns-section">
    <div class="ns-label">Notebooks<button data-notes-action="create-notebook"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></button></div>`;
  notebooksArr.sort((a, b) => (a.order || 0) - (b.order || 0)).forEach(nb => {
    ...
    html += `<div class="ns-item${active}" draggable="true" data-nb-id="${nb.id}" data-notes-action="view-notebook">
      ...
      <button class="nb-edit-btn" data-notes-action="show-nb-menu" data-nb-id="${nb.id}" title="Edit notebook">
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="1"/><circle cx="19" cy="12" r="1"/><circle cx="5" cy="12" r="1"/></svg>
      </button>
    </div>`;
  });
  html += `</div>`;
  ...
  if (mobileToggle) {
    ...
    mobileToggle.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>
      <span class="current-nb">${currentLabel}</span>
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" style="width:12px;height:12px;margin-left:auto"><polyline points="6 9 12 15 18 9"/></svg>`;
  }
  if (mobileDrawer) {
    let drawerHtml = `
...
function updateNlTitle() {
  ...
  if (notesSidebarView === 'trash' && list.length > 0) {
    ...
    extra = `<button data-notes-action="enter-select-mode" title="Select notes" style="margin-left:auto;border:none;background:none;cursor:pointer;padding:2px 4px;border-radius:4px;color:var(--ink-3);display:flex;align-items:center">
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M9 12l2 2 4-4"/></svg>
    </button>`;
  }
...
function renderNoteList() {
  ...
  if (!list.length && !noteSearchQuery) {
    el.innerHTML = `<div style="text-align:center;padding:40px 20px;color:var(--ink-3)">
      <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" style="margin-bottom:10px;opacity:0.4"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
  ...
  if (!note) {
    el.innerHTML = `<div class="notes-editor-empty">
      <div style="text-align:center">
        <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" style="margin-bottom:10px;opacity:0.3"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
    ...
    <div class="mobile-note-header">
      ...
      <div class="mobile-note-actions">
        ...
        <button class="mobile-note-action" data-notes-action="quill-redo" title="Redo">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.13-9.36L23 10"/></svg>
        </button>
        ...
        <button class="mobile-note-action" data-notes-action="toggle-star" title="Star">
          <svg viewBox="0 0 24 24" fill="${starFill}" stroke="currentColor" stroke-width="1.8" style="color:var(--guava-700)"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
        </button>
      </div>
    </div>
    <div class="ne-body">
      <div class="ne-page">
        ...
        <div class="ne-meta">
          <div class="ne-meta-item">
            ${noteNbTag}
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" style="width:11px;height:11px"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
  ...
  if (note.trashed) {
    html = `
      <button data-notes-action="ctx-restore" data-note-id="${noteId}">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/></svg>
        Restore</button>
    ...
    html = `
      ...
      <div class="ctx-submenu">
        <button style="justify-content:space-between">
          <span style="display:flex;align-items:center;gap:8px"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg> Move to</span>
          <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><polyline points="9 18 15 12 9 6"/></svg>
        </button>
